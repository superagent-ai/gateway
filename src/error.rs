...
pub enum GatewayError {
    ...
    Upstream { status: u16, body: bytes::Bytes, content_type: Option<String> },
...
impl GatewayError {
    pub fn capability(msg: impl Into<String>) -> Self {
        Self::Capability { message: msg.into() }
    }

    fn parts(&self) -> (StatusCode, &'static str, String) {
        match self {
            Self::Capability { message } => (StatusCode::BAD_REQUEST, "gateway_capability_error", message.clone()),
            Self::UnknownModel(_) => (StatusCode::NOT_FOUND, "not_found_error", self.to_string()),
            Self::BadRequest(_) => (StatusCode::BAD_REQUEST, "invalid_request_error", self.to_string()),
            Self::Unauthorized => (StatusCode::UNAUTHORIZED, "authentication_error", self.to_string()),
            Self::AllRoutesFailed { .. } => (StatusCode::BAD_GATEWAY, "gateway_routing_error", self.to_string()),
            Self::Network(_) => (StatusCode::BAD_GATEWAY, "gateway_upstream_error", self.to_string()),
    ...
    pub fn into_response_for(self, client: ClientProtocol) -> Response {
        ...
        if let Self::Upstream { status, body, content_type } = self {
